...
function HouseRulesCard() {
  ...
  return (
    <section className="mt-5 overflow-hidden rounded-[1.75rem] border-2 border-white bg-hennies-sky text-hennies-navy shadow-2xl">
      <div className="flex flex-col gap-3 bg-hennies-orange px-5 py-4 text-white sm:flex-row sm:items-center sm:justify-between">
        <div>
        ...
        <div className="w-fit rotate-[-3deg] rounded-2xl bg-white px-4 py-2 text-2xl font-black uppercase text-hennies-navy shadow-[4px_4px_0_rgba(0,47,95,.22)]">
          Rule #1
        </div>
      </div>
      <div className="grid gap-4 p-5 lg:grid-cols-[1fr_300px] lg:p-6">
        <ol className="grid gap-2 text-sm font-black leading-snug sm:grid-cols-2 sm:text-base lg:grid-cols-1">
          {rules.map((rule, index) => (
            <li key={rule} className="flex gap-3">
              <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-white text-xs text-hennies-navy">
                {index + 1}
              </span>
              <span>{rule}</span>
            </li>
        ...
        <div className="rounded-[1.5rem] border border-hennies-navy/15 bg-white/75 p-4 shadow-inner">
          ...
          <p className="mt-2 text-lg font-black leading-tight">
            Lekker food, cold drinks and good gees. Ask your waiter about any
